...
- **Registrierung/Login:** `AuthScreen`-Komponente, ruft `supabaseClient.auth.signUp(...)` bzw. `signInWithPassword(...)` auf. Registrierung erfordert E-Mail-Bestätigung (Supabase schickt einen Bestätigungslink; ohne Bestätigung entsteht keine Session, siehe `AuthScreen`-Infomeldung „Fast geschafft! …").
